...
func AppleLogin(c *gin.Context) {
	...
	if err != nil {
		slog.Warn("apple login missing kid from identity token header")
	...
	if !found {
		slog.Error("Apple identity token kid missing from keys", "kid", kid)
		c.IndentedJSON(http.StatusBadRequest, gin.H{"error": "Invalid jwt token"})
		return
	}
	...
	if valid, err := utils.VerifyRSAToken(authReq.AppleToken, key); !valid || err != nil {
		...
		slog.Warn("Forged Apple identity token NOT generated by Apple")
		c.IndentedJSON(http.StatusBadRequest, gin.H{"error": "Invalid jwt token"})
		return
	}
	...
	if err != nil {
		slog.Error("Failed to check whether user exists by Apple ID", "appleId", userInfo.AppleId, "error", err.Error())
	...
	if !userExists {
		...
		if err != nil {
			slog.Error("Error creating user", "appleId", userInfo.AppleId, "error", err.Error())
			c.IndentedJSON(http.StatusInternalServerError, gin.H{"error": "error creating new user"})
			return
		}
		...
		tokenString, err := utils.CreateToken(*createdUser)
		if err != nil {
			slog.Error("Failed to create token for newly created user", "error", err.Error())
			c.IndentedJSON(http.StatusInternalServerError, gin.H{"error": "error creating token"})
			return
		}

		// Return 201 created
		slog.Info("New Apple Signup", "userId", createdUser.ID, "appleId", createdUser.AppleId)
		c.IndentedJSON(http.StatusCreated, gin.H{"user": createdUser, "token": tokenString})
		return
	}
	...
	tokenString, err := utils.CreateToken(*user)
	if err != nil {
		slog.Error("Failed to create token for existing user", "userId", user.ID)
		c.IndentedJSON(http.StatusInternalServerError, gin.H{"error": "error creating token"})
		return
	}

	slog.Info("Successful Apple login", "userId", user.ID)
	response := LoginResponse{User: user, Token: tokenString}
	c.IndentedJSON(200, response)
}
